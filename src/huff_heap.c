...
static inline int parent(int i) { return i > 0 ? (i - 1) / 2 : i; }
...
static void heapify(MinHeap* h, int i) {
    ...
    if (right < h->size &&
        h->data[right]->probability < h->data[smallest]->probability)
        smallest = right;
...
void huff_minheap_init(MinHeap* h) { h->size = 0; }
...
void huff_minheap_insert(MinHeap* h, HuffmanNode* n) {
    ...
    while (cur > 0 &&
           h->data[cur]->probability < h->data[parent(cur)]->probability) {
...
HuffmanNode* huff_minheap_pop(MinHeap* h) {
    ...
    HuffmanNode* node = huff_minheap_get_min(h);
    huff_minheap_delete_min(h);

    return node;
}

HuffmanNode* huff_minheap_get_min(MinHeap* h) { return h->data[0]; };
